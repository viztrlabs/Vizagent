...
function HealthTab({ health, viewers, formatUptime }: { health: HealthData | null; viewers: ViewersData | null; formatUptime: (s: number) => string }) {
...
function AnalyticsTab({ sessions }: { sessions: SessionData[] }) {
...
function ControlTab({ config, onUpdate }: { config: ConfigData | null; onUpdate: (access: 'full' | 'partial' | 'view-only') => void }) {
  const accessOptions = [
    { value: 'full' as const, label: 'Full Access', desc: 'Mouse + keyboard input enabled' },
    { value: 'partial' as const, label: 'Partial Access', desc: 'Camera rotation only' },
    { value: 'view-only' as const, label: 'View Only', desc: 'No input sent to UE5' },
  ];
