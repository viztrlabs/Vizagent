...
import { Save, RefreshCw, ExternalLink, ChevronRight, Database, Mail, Cloud, Key, AlertTriangle, CheckCircle, Loader2 } from 'lucide-react';

interface EnvVar {
  key: string;
  label: string;
  description: string;
  category: 'database' | 'auth' | 'email' | 'storage' | 'payments' | 'streaming' | 'other';
  required: boolean;
  sensitive: boolean;
  value?: string;
  placeholder?: string;
...
const PROVIDERS = {
  ...
  auth: [
    ...
    { id: 'nextauth', name: 'NextAuth.js', description: 'Custom auth providers', icon: '🔐', envVars: ['NEXTAUTH_SECRET', 'NEXTAUTH_URL', 'GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET'], current: false },
  ],
} as const;

type CategoryKey = keyof typeof CATEGORIES;
type EnvVarType = typeof ENV_VARS[number];
type ProviderType = typeof PROVIDERS[keyof typeof PROVIDERS][number];
...
export function AdminEnvSettingsClient() {
  ...
  const categoryVars = ENV_VARS.filter(v => v.category === activeCategory);
          ...
          <div className="p-4 border-t border-gray-700/50">
            ...
            <div className="space-y-2">
              {Object.entries({
                ...
                auth: [
                  { id: 'supabase-auth', name: 'Supabase Auth', description: 'Built-in authentication', icon: '🔐', envVars: ['NEXT_PUBLIC_SUPABASE_URL', 'NEXT_PUBLIC_SUPABASE_ANON_KEY', 'SUPABASE_SERVICE_ROLE_KEY'], current: true },
                  { id: 'nextauth', name: 'NextAuth.js', description: 'Custom auth providers', icon: '🔐', envVars: ['NEXTAUTH_SECRET', 'NEXTAUTH_URL', 'GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET'] },
                ],
