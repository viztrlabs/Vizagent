...
export function UploadDropzone({ projectId, onUploadComplete, className }: UploadDropzoneProps) {
  ...
  const uploadToSupabase = useCallback(async (file: File, uploadUrl: string, storagePath: string) => {
  ...
  }, [validateFile, getPresignedUrl, uploadToSupabase, projectId, onUploadComplete]);
